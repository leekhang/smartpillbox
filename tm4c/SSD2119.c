...
void LCD_GPIOInit(void){
    ...
    RCGCGPIO |= RCGCGPIO_B; // activate port B
    wait++; wait++;         // wait for port activation
    GPIODIR_B |= 0xFF;      // make PB0-7 outputs
    GPIOAFSEL_B &= ~0xFF;   // disable alternate functions
    GPIODEN_B = 0xFF;      // enable digital I/O on PB0-7

    // activate control pins
    RCGCGPIO |= RCGCGPIO_A; // activate port A
    wait++; wait++;         // wait for port activation
    GPIODIR_A |= 0xF0;      // make PA4-7 outputs
    GPIOAFSEL_A &= ~0xF0;   // disable alternate functions for A
    GPIODEN_A = 0xF0;      // enable digital I/O on PA4-7
